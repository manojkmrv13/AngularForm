...
@Component({
    selector: 'completed-needs-details',
    templateUrl: './completed-needs-detail.component.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['./completed-needs-detail.component.css']
})

export class CompletedNeedsDetailsComponent implements OnInit, AfterViewInit {
    ...
    @Input() TwiterText: string;
    @Input() pinterestMedia: string;
    @Input() pinterestText: string;
    @ViewChild('element', {static: false}) element: ElementRef;
    toggle() {
        $('#shareListwrp').toggleClass('show');
    }
    ...
    constructor(
        ...
        if (this.isBrowser) {
            window['completedNeedsDetailsComponentRef'] = { component: this, zone: _ngZone };
            window['jQuery'] = $;
            window['$'] = $;
        ...
        //         $(".SuccessStories, .Testimonials, .SimilarNeedsSlider").slick('unslick');
        //         this.RunOnInit()  
    ...
    postFundraising(e) {
        ...
        //     $("#need" + e.target.value).parents('.btnStartFundraising').find('span').text('Added your Fundraising');
        // } else {
        //     this.needIdForFundraising.splice(index, 1);
        //     $("#need" + e.target.value).parents('.btnStartFundraising').find('span').text('Start your Fundraising');
        // }
        ...
        if (this.DONORID > 0) {
            if (ngForm.valid) {
                ...
                this.http.post(Globalvar.ApiUrl + "/PostWebsiteComments", body).subscribe((data) => {
                    ...
                    this.COMMENT = "";
                    setTimeout(() => {
                        alert("Comment has been submitted successfully. It will be posted once approved.")
                    }, 200)
                });
            } else {
    ...
    DonateNow(item) {
        ...
        $("." + item).each(function () {
            if ($(this).val() > 0 && $(this).val() !== '') {
                var cartNeedId = parseInt($(this).attr("id").replace('need', ''));
    ...
    GetCartNeedItem() {
        this.CartS.CartService(this.CartDonorID).subscribe((CartNeedData) => {
            CartNeedData.forEach(element => {
                setTimeout(function () {
                    $("#need" + element.NEEDID).val(element.AMOUNT)
                    $("#giftQuantity" + element.NEEDID).val(element.AMOUNT)
    ...
    GetNeeds(needID) {
        this.nds = null;
        this.ns.getCompletedNeeds(needID, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "").subscribe((nds) => {
            //console.log("nds", nds)
            if (nds.length >= 0) {
                if (nds[0].NEEDID >= 0) {
                    this.nds = nds;
                    ...
                    //console.log('NEEDID: '+ this.needIDComment);
                    this.NEEDTYPEID = nds[0].NEEDTYPEID;
                    this.NEEDTYPE = nds[0].NEEDTYPE;
                    //console.log('NEEDTYPEID: ' + this.NEEDTYPEID);
                    ...
                    // this.SVGCircleAnimation('.circleWrp', 200, 3000, 100, this.ProjectsCost, nds[0].ADMINISTRATION_CHARGES, 90);
                    this.TwiterText = nds[0].NEED_DESCRIPTION;
                    this.pinterestMedia = nds[0].NEED_THUMBNAIL_ACTUAL_IMAGE_FILEPATH;
                    this.pinterestText = nds[0].NEED_DESCRIPTION;

                    this.shareTitle = nds[0].NEED_NAME;
                    this.shareText = nds[0].NEED_DESCRIPTION;
    ...
    needShareDropdownF(title, text, media) {
        ...
        new needShareDropdown(document.getElementById('shareButtonC'), {
    ...
    //     this.clp = (100 - (((NEED_AMOUNT - ADMINISTRATION_CHARGES) / NEED_AMOUNT) * 100)).toFixed(2);
    //     return this.clp;
    //     //return (100 - (((NEED_AMOUNT - ADMINISTRATION_CHARGES) / NEED_AMOUNT) * 100)).toFixed(2);
    // };
    CalculatePercentage(NEED_AMOUNT, ADMINISTRATION_CHARGES) {
    ...
    // SVGCircleAnimation(svgwrp, pLth, speed, circleShape1, circleShape2, circleShape3, rotate) {
    //     var pLength = pLth / 2,
    //         radius1 = pLength - 10,
    //         radius2 = pLength - 30,
    //         radius3 = pLength - 50,
    //         dAr1 = Math.round(2 * Math.PI * radius1),
    //         dAr2 = Math.round(2 * Math.PI * radius2),
    //         dAr3 = Math.round(2 * Math.PI * radius3),
    //         sOffset1 = dAr1 - (circleShape1 / 100 * dAr1),
    //         sOffset2 = dAr2 - (circleShape2 / 100 * dAr2),
    //         sOffset3 = dAr3 - (circleShape3 / 100 * dAr3),
    //         speed, circleShape1, circleShape2, circleShape3;
    //     setTimeout(function () {
    //         $(svgwrp).css({ 'width': pLth, 'height': pLth });
    //         $(svgwrp).find('svg').css({ 'width': pLth, 'height': pLth, '-webkit-transform': 'rotate(' + rotate + 'deg)', 'transform': 'rotate(' + rotate + 'deg)' });
    //         $('#svgCircleShape1').css({ 'cy': pLength, 'cx': pLength, 'r': radius1, 'fill': 'none', 'stroke': '#cccccc', 'stroke-dasharray': dAr1, 'stroke-dashoffset': 0 });
    //         $('#svgCircleShape2').css({ 'cy': pLength, 'cx': pLength, 'r': radius2, 'fill': 'none', 'stroke': '#cccccc', 'stroke-dasharray': dAr2, 'stroke-dashoffset': 0 });
    //         $('#svgCircleShape3').css({ 'cy': pLength, 'cx': pLength, 'r': radius3, 'fill': 'none', 'stroke': '#cccccc', 'stroke-dasharray': dAr3, 'stroke-dashoffset': 0 });
    //     }, 100);
    //     setTimeout(function () {
    //         $('#svgCircle1').css({ 'cy': pLength, 'cx': pLength, 'r': radius1, 'fill': 'none', 'stroke': '#FF9100', 'stroke-dasharray': dAr1, 'stroke-dashoffset': dAr1 }).animate({ 'stroke-dashoffset': sOffset1 }, speed);
    //         $('#svgCircle2').css({ 'cy': pLength, 'cx': pLength, 'r': radius2, 'fill': 'none', 'stroke': '#FF9100', 'stroke-dasharray': dAr2, 'stroke-dashoffset': dAr2 }).animate({ 'stroke-dashoffset': sOffset2 }, speed);
    //         $('#svgCircle3').css({ 'cy': pLength, 'cx': pLength, 'r': radius3, 'fill': 'none', 'stroke': '#FF9100', 'stroke-dasharray': dAr3, 'stroke-dashoffset': dAr3 }).animate({ 'stroke-dashoffset': sOffset3 }, speed);
    //     }, 1000);
    // }
    ...
    customeSelectControl(DA, nID, e) {
        var dAmount = DA, thisV = e.target.value, targetID = nID;
        $("#need" + targetID).val(thisV)
    }
    public amount;
    donationAmount(reminingA, minimumA, e) {
        if(e.target.value !== '' && e.target.value !== undefined){
        if (reminingA >= e.target.value && minimumA <= e.target.value) {
            this.amount = e.target.value
        } else {
            if (reminingA < e.target.value) {
                e.target.value = 0
                alert(' Contribution amount should be less than ' + reminingA)
            }
            if (minimumA > e.target.value) {
                e.target.value = e.target.value
                alert(' Contribution amount should be greater than or equal ' + minimumA)
            }
        }
    }
    }
    ...
    ngOnInit() {
        ...
        this.needID = this.activeRoute.snapshot.paramMap.get('id');
        console.log("test: "+this.needID);
        if( !this.isBrowser ){
            this.ns.getNeed(this.needID).subscribe((nds) => {
                console.log(this.needID);
                if (nds.length > 0) {
                    if (nds[0].NEEDID > 0) {
                        this.nds = nds;
                        let title = "HoSh - "+nds[0].NEED_NAME;
                        let description = nds[0].META_DESCRIPTION !=null && nds[0].META_DESCRIPTION != "" ? nds[0].META_DESCRIPTION : nds[0].NEED_SHORT_DESCRIPTION;
    ...
    ngAfterViewInit() {
        ...
        //window['gapi'] && window['gapi'].plusone.go(this.pinterestText);
        // render pin it button
        //window['parsePins'] && window['parsePins']();
        // add linkedin share button script tag to element
        // this.element.nativeElement.innerHTML = `<script type="IN/Share" data-url="${this.lkdurl}" data-counter="top"></script>`;  
        // render share button
        ...
        // setTimeout(function () {
        //     this.ScrollPageHt = $('.PHeaderWrp').outerHeight() + $('.PSliderWrp').outerHeight() + $('.PLisitingWrp').outerHeight();
        //     $('.Educationtabs li').on('click', function () {
        //         console.log('click');
        //         var scrollAnchor = $(this).attr('data-scroll'),
        //             scrollPoint = $('div[data-anchor="' + scrollAnchor + '"]')
        //         $('body,html').animate({
        ...
        //         if (windscroll > this.ScrollPageHt) {
        //             $('.EducationSmartSchool').addClass('fixed');
        //         } else {
        //             $('.EducationSmartSchool').removeClass('fixed');
        //         }
        //         if ($(this).scrollTop() < $('div[data-anchor="Update"]').offset().top + 40) {
        //             $('.Educationtabs li').removeClass('active');
        //             $('.Educationtabs li:eq(0)').addClass('active');
        //         }
        //         if ($(this).scrollTop() < $('div[data-anchor="comments"]').offset().top + 40) {
        //             $('.Educationtabs li').removeClass('active');
        //             $('.Educationtabs li:eq(0)').addClass('active');
        //         }

        //         if ($(this).scrollTop() >= $('div[data-anchor="comments"]').offset().top - 160) {
        //             $('.Educationtabs li').removeClass('active');
        //             $('.Educationtabs li:eq(1)').addClass('active');
        //         }
        //         if ($(this).scrollTop() >= $('div[data-anchor="supporters"]').offset().top - 130) {
        //             $('.Educationtabs li').removeClass('active');
        //             $('.Educationtabs li:eq(2)').addClass('active');
        //         }
        //     });
        //     console.log(this.ScrollPageHt);
        // }, 2000)
        /* var taboffset = $('.Educationtabs').offset().top;
         $(window).scroll(function() {
            var windscroll = $(window).scrollTop();
            if (windscroll >= taboffset) {
                $('.Educationtabs').addClass('fixed');
                $('.Wapper section').each(function(i) {
                    if ($(this).position().top <= windscroll - 20) {
                        $('.Educationtabs li.active').removeClass('active');
                        $('.Educationtabs li').eq(i).addClass('active');
                    }
                });
        
            } else {        
                $('.Educationtabs').removeClass('fixed');
                $('.Educationtabs li.active').removeClass('active');
                $('.Educationtabs li:first').addClass('active');
            }        
        }); */
